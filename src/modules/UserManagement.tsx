...
export default function UserManagement() {
  ...
  const allPlanos = useMemo(() => users.flatMap(u => (u.planos || []).map(p => ({ ...p, userName: u.nome || u.username }))), [users]);
  const totalReceitaMensal = useMemo(() => allPlanos.reduce((s, p) => s + Number(p.valor), 0), [allPlanos]);
  ...
  // Chart: revenue per user
  const revenueByUser = useMemo(() => {
    const map: Record<string, number> = {};
    allPlanos.forEach(p => { map[p.userName] = (map[p.userName] || 0) + Number(p.valor); });
    return Object.entries(map).map(([name, valor]) => ({ name, valor }));
  }, [allPlanos]);

  // Chart: plan status distribution
  const statusData = useMemo(() => [
    { name: 'Ativos', value: planosAtivos.length, color: 'hsl(var(--primary))' },
    { name: 'Vencidos', value: planosVencidos.length, color: 'hsl(var(--destructive))' },
  ].filter(d => d.value > 0), [planosAtivos, planosVencidos]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 border-b pb-2">
        <button
          ...
          className={`flex items-center gap-1.5 rounded-t px-3 py-1.5 text-sm font-medium transition-colors ${mainTab === 'dashboard' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
        >
          <BarChart3 className="h-4 w-4" /> Dashboard
        </button>
        <button
          onClick={() => setMainTab('usuarios')}
          className={`flex items-center gap-1.5 rounded-t px-3 py-1.5 text-sm font-medium transition-colors ${mainTab === 'usuarios' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}
        >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Card>
              <CardContent className="p-3 text-center">
                ...
                <p className="text-2xl font-bold">R$ {totalReceitaMensal.toFixed(0)}</p>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            {revenueByUser.length > 0 && (
              <Card>
                <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-1"><BarChart3 className="h-4 w-4" /> Receita por Usuário</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-52">
            ...
            {statusData.length > 0 && (
              <Card>
                <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-1"><CreditCard className="h-4 w-4" /> Status dos Planos</CardTitle></CardHeader>
                <CardContent>
