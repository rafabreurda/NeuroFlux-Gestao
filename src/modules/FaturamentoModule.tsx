...
import { Recibo, OrdemServico, Orcamento, CustoFixo } from '@/types';
...
export default function FaturamentoModule({ recibos, addRecibo, empresaLogo, empresaNome, ordens, orcamentos, custos }: Props) {
  ...
  const financeiro = useMemo(() => {
    ...
    const porOS = ordens.map(os => {
      ...
      const custoOS = orcamentosOS.reduce((s, o) => {
        return s + o.materiais.reduce((sm, m) => sm + m.valor, 0);
      }, 0);
      const lucroOS = entradaOS - custoOS;
      return {
        codigo: os.codigo,
    ...
    recibos.forEach(r => {
      porPagamento[r.formaPagamento] = (porPagamento[r.formaPagamento] || 0) + r.valor;
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Card className="border-l-4 border-l-green-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
              <ArrowUpRight className="h-3.5 w-3.5 text-green-500" /> Entradas
            </div>
        ...
        <Card className="border-l-4 border-l-red-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
              <ArrowDownRight className="h-3.5 w-3.5 text-red-500" /> Saídas
            </div>
        ...
        <Card className={`border-l-4 ${lucroPositivo ? 'border-l-blue-500' : 'border-l-orange-500'}`}>
          <CardContent className="p-4">
            ...
            <p className={`mt-1 text-xl font-bold ${lucroPositivo ? 'text-blue-600' : 'text-orange-600'}`}>
              R$ {financeiro.lucro.toFixed(2)}
            </p>
        ...
        <Card className="border-l-4 border-l-amber-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
              <BarChart3 className="h-3.5 w-3.5 text-amber-500" /> Orçamentos
            </div>
      ...
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="dashboard" className="space-y-4 mt-4">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={financeiro.chartPagamento} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={10}>
                        {financeiro.chartPagamento.map((_, i) => (
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold">Resumo Rápido</CardTitle>
            </CardHeader>
            <CardContent>
